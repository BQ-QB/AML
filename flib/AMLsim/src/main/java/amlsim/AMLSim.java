...
import it.unimi.dsi.parser.callback.DebugCallbackDecorator;
...
public class AMLSim extends SimState {
	...
	// accounts
	// private float sarBalanceRatio = 10.0F; // Multiplier of initial balance for
	// SAR accounts
	...
	public void loadParametersFromFile() {
		...
		int suspicious_in_int = simProp.getCashTxInterval(true, true); // Interval of cash-in transactions for
																		// suspicious account
		float norm_in_min = simProp.getCashTxMinAmount(true, false); // Minimum amount of cash-in transactions for
																		// normal account
		float norm_in_max = simProp.getCashTxMaxAmount(true, false); // Maximum amount of cash-in transactions for
																		// normal account
		float suspicious_in_min = simProp.getCashTxMinAmount(true, true); // Minimum amount of cash-in transactions for
																			// suspicious account
		float suspicious_in_max = simProp.getCashTxMaxAmount(true, true); // Maximum amount of cash-in transactions for
																			// suspicious account
		CashInModel.setParam(norm_in_int, suspicious_in_int, norm_in_min, norm_in_max, suspicious_in_min,
				suspicious_in_max);

		int norm_out_int = simProp.getCashTxInterval(false, false); // Interval of cash-out transactions for normal
																	// account
		int suspicious_out_int = simProp.getCashTxInterval(false, true); // Interval of cash-out transactions for
																			// suspicious account
		float norm_out_min = simProp.getCashTxMinAmount(false, false); // Minimum amount of cash-out transactions for
																		// normal account
		float norm_out_max = simProp.getCashTxMaxAmount(false, false); // Maximum amount of cash-out transactions for
																		// normal account
		float suspicious_out_min = simProp.getCashTxMinAmount(false, true); // Minimum amount of cash-out transactions
																			// for suspicious account
		float suspicious_out_max = simProp.getCashTxMaxAmount(false, true); // Maximum amount of cash-out transactions
																			// for suspicious account
	...
	private void loadNormalModelsFile(String filename) throws IOException {
		try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				int startStep = Integer.parseInt(elements[columnIndexMap.get("startStep")]);
				int endStep = Integer.parseInt(elements[columnIndexMap.get("endStep")]);

				AccountGroup accountGroup;
				if (this.accountGroupsMap.containsKey(accountGroupId)) {
					accountGroup = this.accountGroupsMap.get(accountGroupId);
				} else {
					accountGroup = new AccountGroup(accountGroupId, startStep, endStep, scheduleID,
							this.normalTxInterval, this);
				...
				switch (type) {
					case AbstractTransactionModel.SINGLE:
						model = new SingleTransactionModel(accountGroup, rand);
						break;
					case AbstractTransactionModel.FAN_OUT:
						model = new FanOutTransactionModel(accountGroup, rand);
						break;
					case AbstractTransactionModel.FAN_IN:
						model = new FanInTransactionModel(accountGroup, rand);
						break;
					case AbstractTransactionModel.MUTUAL:
						model = new MutualTransactionModel(accountGroup, rand);
						break;
					case AbstractTransactionModel.FORWARD:
						model = new ForwardTransactionModel(accountGroup, rand);
						break;
					case AbstractTransactionModel.PERIODICAL:
						model = new PeriodicalTransactionModel(accountGroup, rand);
						break;
					default:
			...
			for (AccountGroup accountGroup : accountGroupsMap.values()) {
				accountGroup.getModel().setParameters();
			}
		}
	}
		...
		while ((line = reader.readLine()) != null) {
			...
			if (minAmount > maxAmount) {
				throw new IllegalArgumentException(
						String.format("minAmount %f is larger than maxAmount %f", minAmount, maxAmount));
			}
			if (startStep > endStep) {
				throw new IllegalArgumentException(
						String.format("startStep %d is larger than endStep %d", startStep, endStep));
			}
	...
	/**
	 * Manage a transaction for logging and diameter computation of the whole
	 * transaction network
	 * 
